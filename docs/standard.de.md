...
### Grenze für delegierten Besprechungszugriff

Eine Gastfreigabe für eine Besprechung bleibt auf ihre `meeting`-Ressource beschränkt; das Share-Gateway behandelt sie niemals als Whiteboard-Freigabe. Delegierter Board-Zugriff setzt voraus, dass die ctx-Fähigkeit `meetings:resolveWhiteboardAssociation` die Jitsi-eigene Zuordnung zwischen Besprechung und Whiteboard bestätigt und den angeforderten Vorgang `whiteboard:read` oder `whiteboard:write` ausdrücklich erlaubt. Anschließend lässt Whiteboard die ursprüngliche Besprechungsfreigabe und deren Berechtigung `meeting:join` unabhängig durch `share:resolveGuestAccess` prüfen. Fehlende, widerrufene, unpassende oder unzureichend eingeschränkte Ergebnisse verweigern den Zugriff auf Sitzungen, Elemente, Anwesenheit, Bilder und Startrouten.
